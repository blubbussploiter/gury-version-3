...
{
	Rect2D prev = renderDevice->getViewport();
	renderDevice->notifyResize(viewportWidth, viewportHeight);
	Rect2D viewportRect = Rect2D::xywh(prev.x0(), prev.y0(), renderDevice->getWidth(), renderDevice->getHeight());
	renderDevice->setViewport(viewportRect);
	window->notifyResize(viewportWidth, viewportHeight);
}
